...
function stud_tabler(){      
    ...
    $result = $conn->query($sql);
    echo "<table class=\"table text-white\"><tr><th>Subject</th></tr>";
    if ($result->num_rows == 0) {
            ...
            <?php elseif ($_SESSION['type']==="STUDENT"): ?>
                ...
                <h2>Enroll Subject</h2>
                    <form id="studentEnrollSubject" method="POST" action="./subjects.php">
                            ...
                            <button id="studentEnrollButton" class="btn l-green" name="submit">Enroll</button>
                </div>
            <?php endif?>
                    ...
                    <?php
                        ...
                        while ($row=$result->fetch_assoc()) : 
                            ...
                            if ($isConflict==1 || $row6['COUNT(*)']>=$row5['maximum_population']) : 
                                $isConflict = 0; ?>
                                <li class="invalid">
                                Group Number: <?php echo $row['id'] ?>&ensp;
                          <?php while ($row4=$result4->fetch_assoc()) : ?>
                                    <?php echo $row4['day_of_week'] ?>&ensp;<?php echo $row4['start'] ?>-<?php echo $row4['end'] ?>&emsp;
                          <?php endwhile ?>
                      <?php else: ?>
                                <li class="valid">
                                Group Number: <?php echo $row['id'] ?>&ensp;
                          ...
                          <?php endwhile ?>
                      <?php endif ?>
                      &emsp;<?php echo $row6['COUNT(*)'] ?>/<?php echo $row5['maximum_population'] ?>
                            </li>
